...
country, sex, age, suicides_per_100k, country_development

Since prices of houses varies from 50k to 7M, we need to have them as discretized for decision tree building. We split them into 5 chunks with equal sizes. And add a new column named as `FiyatAraligi` to mark each row from one of price intervals.

```{r}
library(Hmisc)
chunks <- cut2(suicide_rates$suicides_per_100k, g = 5)
#chunks <- cut(suicide_rates$suicides_per_100k, breaks = 4)
#levels(chunks) <- c("0.0-1.20", "1.20-6.23", "6.23-16.88", "16.88-204.92")
...
Since too many features increase the complexity of tree, we trimmed down some of columns.

```{r}
# data$MustakilMi <- as.factor(data$MustakilMi)
# data$Manzarali <- data[, "ManzaraDoga"] | data[, "ManzaraGol"] | data[, "ManzaraDeniz"] | data[, "ManzaraBogaz"]
# data$Manzarali <- as.factor(data$Manzarali)
# data$YukseklikOrani <- data[, "BulunduguKat"] / data[, "ToplamKat"]
```

## Building a decision tree

We are planning to build our tree using this formula:
`FiyatAraligi ~ MustakilMi  + SalonSayisi +  ToplamOdaSayisi + GercekYas + OrijinalAlan + Isitma + Manzarali + CepheGuney +  YukseklikOrani + BanyoSayisi`

We omitted  `Mahalle` since it contains 377 levels and it disables the algorithm to build a tree fast.
...
```{r}
t1 <- rpartXse( suicides_per_100k ~  sex + age + country_development + generation + year , se = 0, train_set, cp = 0.005,  maxdepth = 20  
                , model = TRUE)
...
```{r}
t2 <- rpartXse( suicides_per_100k ~  sex + age + country_development, se = 0, suicide_rates, cp = 0.005,  maxdepth = 20  
                , model = TRUE)
prp(t2, type = 0, extra = 101)
```

```{r eval=FALSE}
library(adabag)
bag <- bagging(suicides_per_100k ~  sex + age + country_development, train_set[1:10,], nbagg = 2, coob = TRUE, control = rpart.control(minsplit = 2, cp = 0)) #, maxdepth=1 mfinal = 1
...
forest <- randomForest(rate_interval ~  sex + age + country_development  + generation + year , train_set, ntree = 50, mtry = 2) # mtry feature subsets
...
- When we observe our tree, we notice 3 columns used for building our tree: `OrijinalAlan`, `ToplamOdaSayisi`, `BanyoSayisi` and `Ilce`. It's reasonable that the first and greater split was made with total square meters of house. And then, the second most significant split is determined with `Ilce` which is very closely linked with the  price of houses. And total room number and number of bathrooms are the last indicators.
